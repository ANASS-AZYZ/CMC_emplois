<x-app-layout>
            ...
            <div class="bg-white p-8 shadow-sm sm:rounded-lg border border-slate-700/40">
                <form action="{{ route('groupes.store') }}" method="POST">
                    ...
                    <div class="mb-4">
                        ...
                        <select id="group-filiere" name="filiere_id" class="w-full rounded-md border border-slate-400 bg-white px-4 py-3 text-slate-900 font-semibold shadow-sm" required>
                            @foreach($filieres as $filiere)
                                <option
                                    value="{{ $filiere->id }}"
                                    data-niveau="{{ $filiere->niveau }}"
                                    data-nom="{{ $filiere->nom }}"
                                    {{ (string) old('filiere_id') === (string) $filiere->id ? 'selected' : '' }}
                                >
                                    {{ $filiere->nom }}
                                </option>
                            @endforeach
                    ...
                    <div class="mb-4">
                        ...
                        <input id="group-code" type="text" name="code" value="{{ old('code') }}" class="w-full rounded-md border border-slate-400 bg-white px-4 py-3 text-slate-900 font-semibold placeholder-slate-500 shadow-sm" placeholder="Ex: DES-1" required>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            ...
            if (!yearSelect || !filiereSelect || !codeInput) {
                return;
            }
